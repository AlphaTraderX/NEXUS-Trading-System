...
Sources: ForexFactory JSON API (via forex_factory.py)
"""
...
def _convert_ff_event(ff_event) -> EconomicEvent:
    """Convert a ForexFactory EconomicEvent to our local EconomicEvent."""
    from nexus.data.forex_factory import EventImpact as FFImpact

    impact_map = {
        FFImpact.HIGH: EventImpact.HIGH,
        FFImpact.MEDIUM: EventImpact.MEDIUM,
        FFImpact.LOW: EventImpact.LOW,
    }
    return EconomicEvent(
        time=ff_event.event_time,
        currency=ff_event.currency,
        impact=impact_map.get(ff_event.impact, EventImpact.LOW),
        event=ff_event.event_name,
        forecast=ff_event.forecast,
        previous=ff_event.previous,
        actual=ff_event.actual,
    )
...
class EconomicCalendar:
    ...
    Data is fetched from ForexFactory via forex_factory.py client,
    with a fallback to known recurring events (e.g. NFP first-Friday).
    """
    ...
    async def fetch_calendar(self, target_date: Optional[date] = None) -> List[EconomicEvent]:
        ...
        # Fallback to known recurring events if nothing fetched
        if not events:
    ...
    async def _fetch_forex_factory(self, target: date) -> List[EconomicEvent]:
        ...
        ff_events = await ff.get_events_for_date(target)
        return [_convert_ff_event(e) for e in ff_events]

    def _get_known_events(self, target: date) -> List[EconomicEvent]:
        """Get known scheduled events (fallback when API is unreachable)."""
        events = []
        ...
        if target == first_friday:
            events.append(EconomicEvent(
                time=datetime.combine(target, time(13, 30)),  # 8:30 AM ET = 13:30 UTC
                currency="USD",
